            ...
            <div class="row">
                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    ...
                                    @foreach ($product->galleries as $gallery)
                                        <div class="swiper-slide product-single__image-item">
                                            <img loading="lazy" class="h-auto" src="{{ Storage::url($gallery->image) }}"
                                                width="674" height="674" alt="" />
                                            <a data-fancybox="gallery" href="{{ Storage::url($gallery->image) }}"
                                                data-bs-toggle="tooltip" data-bs-placement="left" title="Zoom">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <use href="#icon_zoom" />
                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                                            src="{{ Storage::url($product->image) }}" width="104" height="104"
                                            alt="" /></div>
                                    @foreach ($product->galleries as $gallery)
                                        <div class="swiper-slide product-single__image-item"><img loading="lazy"
                                                class="h-auto" src="{{ Storage::url($gallery->image) }}" width="104"
                                                height="104" alt="" /></div>
                                    @endforeach
                                </div>
                            </div>
                ...
                <div class="col-lg-5">
                    ...
                    @if (\Surfsidemedia\Shoppingcart\Facades\Cart::instance('cart')->content()->where('id', $product->id)->count() > 0)
                        <a href="{{ route('user.cart.index') }}" class="btn btn-primary btn-addtocart ">Go to Cart</a>
                    @else
                        <form name="addtocart-form" method="post" action="{{ route('user.cart.add') }}">
                            @csrf
                    ...
                    <div class="product-single__addtolinks">
                        @if (isset($wishlistItems) && array_key_exists($product->id, $wishlistItems))
                            <form method="POST"
                                action="{{ route('user.wishlist.destroy', ['rowId' => $wishlistItems[$product->id]]) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="menu-link menu-link_us-s add-to-wishlist border-0 bg-transparent filled-heart">
                        ...
                        @else
                            <form method="POST" action="{{ route('user.wishlist.store') }}">
                                @csrf
                                <input type="hidden" name="id" value="{{ $product->id }}">
                                <button type="submit"
                                    class="menu-link menu-link_us-s add-to-wishlist border-0 bg-transparent">
                                    <svg width="16" height="16" viewBox="0 0 20 20" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <use href="#icon_heart" />
                                    </svg>
                        ...
                        @foreach ($rproducts as $rproduct)
                            <div class="swiper-slide product-card">
                                <div class="pc__img-wrapper">
                                    <a
                                        href="{{ route('user.shop.product-details', ['product_slug' => $rproduct->slug]) }}">
                                    ...
                                    @else
                                        <form name="addtocart-form" method="post"
                                            ...
                                            <input type="hidden"
                                                name="price"value="{{ $rproduct->sale_price == '' ? $rproduct->price : $rproduct->sale_price }}">
                                            <button
                                ...
                                <div class="pc__info position-relative">
                                    ...
                                    <div class="product-card__price d-flex">
                                        <span class="money price">
                                            @if ($rproduct->sale_price)
                                                <s>${{ $rproduct->price }}</s> ${{ $rproduct->sale_price }}
                                            @else
                                                ${{ $rproduct->price }}
                                            @endif
                                        </span>
                                    ...
                                    @if (isset($wishlistItems) && array_key_exists($rproduct->id, $wishlistItems))
                                        <form method="POST"
                                            action="{{ route('user.wishlist.destroy', ['rowId' => $wishlistItems[$rproduct->id]]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
